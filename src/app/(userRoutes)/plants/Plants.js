...
import { useClients, usePlants } from "../getApiResponse";
...
const Plants = () => {
  ...
  const createNewPlant = async (payload) => {
    console.log("payload is ", payload);
    try {
      const response = await ApiFetcher("createPlant", payload);
  ...
  const deletePlant = async (id) => {
    try {
      const response = await ApiFetcher("deletePlant", {}, id);
      if (response?.statusCode === 400) {
        PrintLogs("error is post request");
        return;
      }
  ...
  return (
    <div>
      <PageTitle mainTitle={"Admin"} pageTitle={"Plants"} subTitle={"Client"} />
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {loader ? (
                      ...
                      paginatedPlants.map((plant, index) => (
                        ...
                        >
                          <td className="whitespace-nowrap py-4 ps-4 pe-3 text-sm text-center  ">
                            <b>
                              {" "}
                              {(currentPage - 1) * itemsPerPage + index + 1}
                            </b>
                          </td>
                          <td className="whitespace-nowrap py-4 pe-3 text-sm text-center ">
                            {plant.plantName}
                          </td>
      ...
      {isModalOpen && (
        <Modal
          ...
          title={PlantForm.heading}
          containerClassName="text-base
            w-[50%] h-[60%] max-h-[70vh]
          ...
          <div className="pt-4 h-full overflow-y-scroll is-scrollbar-hidden px-2">
            ...
            <Form
              ...
              subClassName2={`border-slate-300 bg-transparent px-3 pb-2
                  ...
                  focus:outline-none
                  `}
              formLayout={PlantForm.PlantsFromMapping}
            />
          </div>
        </Modal>
      )}
        ...
        <div className="pt-4 h-full overflow-y-scroll is-scrollbar-hidden px-2">
          ...
          <Form
            ...
            subClassName2={`border-slate-300 bg-transparent px-3 pb-2
                  ...
                  focus:outline-none
                  `}
            formLayout={updatePlantForm.PlantsFromMapping}
          />
        </div>
      </Modal>
      ...
      >
        <div className="mt-4">
          <p className="text-sm md:text-base mb-6">
            ...
            <span className="font-bold">{isModalOpenDelete?.plantName}</span>?
          </p>
